...
const LabSettingsForm: React.FC = () => {
  ...
  const handleLogoUpload = async () => {
    const logoDataUrl = await window.electronAPI.openFile();
    if (logoDataUrl) {
      setValue('logo', logoDataUrl);
    }
  };
        ...
        <Grid container spacing={3}>
          <Grid item xs={12}>
            <Controller
              ...
              render={({ field }) => (
                <TextField {...field} label="Nombre del Laboratorio" fullWidth />
              )}
            />
          </Grid>
          <Grid item xs={12}>
          ...
          <Grid item xs={12} container spacing={2} alignItems="center">
            <Grid item>
              <Button variant="outlined" onClick={handleLogoUpload}>Subir Logo</Button>
            </Grid>
            <Grid item>
              {logoValue && <Avatar src={logoValue} sx={{ width: 56, height: 56 }} />}
            </Grid>
          </Grid>
           <Grid item xs={12} sm={6}>
            <Controller
