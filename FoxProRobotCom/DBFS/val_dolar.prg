...
DO WHILE inx <= 3
   IF vls(inx) > 0      && Verifica que haya valores
      *** Convierte Centenas
      ...
      IF qty = 1
         IF inx =  3
	        letras = TRIM(letras) + IIF(val_cnv > 100, " UN DOLARES DE LOS ESTADOS UNIDOS DE AMERICA ",rbr(inx,1))
         ELSE
...
*            letras = TRIM(letras) + rbr(inx,2)
   ...
   ELSE
      IF inx = 3 .and. m->camount = "000000000"
         letras = TRIM(letras) + " CERO DOLARES" 
         pase = .F.
      ENDIF
   ENDIF
   inx = inx + 1
...
IF PASE
	DO CASE 
	   CASE vls(2) = 0 AND vls(3)=0 
	      letras = TRIM(letras)+ " DE DOLARES DE LOS ESTADOS UNIDOS DE AMERICA"
	   CASE vls(3)=0 
*	      letras = TRIM(letras)+ " DOLARES"
	      letras = TRIM(letras)
	ENDCASE
ENDIF
********************************************************************************
...
DO CASE
   CASE qty = 0
      letras = TRIM(letras) + " DOLARES DE LOS ESTADOS UNIDOS DE AMERICA"
   CASE qty = 1
      letras = TRIM(letras) + ' 01/100' + ult_str +"DE LOS ESTADOS UNIDOS DE AMERICA"  &&" CON UN CENTAVO"
   OTHERWISE
      letras = TRIM(letras) + ' ' + PADL(qty,2,'0')+'/100' + ult_str +"DE LOS ESTADOS UNIDOS DE AMERICA" &&" CON UN CENTAVO"
